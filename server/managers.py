...
from frontend import (
    ...
    Dron,
    ChangeStateDron,
    StatusMessage as StatusMessageFrontend,
)
from pillar_station import StatusMessage as StatusMessagePillarStation, SetModePillar
...
from db_work import (
    ...
    update_dron,
    get_pillar_station_by_pillar_id,
)
...
class DefaultManager:
    ...
    async def _send_status_frontend(
        self, websocket: WebSocket, status: Literal["Ok", "Err"], message: str
    ):
        await websocket.send_json(
            StatusMessageFrontend(status=status, message=message).model_dump()
        )

    async def _send_message_dron_station(
        self, websocket: WebSocket, name_event: str, data: Any
    ):
        await websocket.send_json(BaseMessage(event=name_event, data=data).model_dump())

    async def _send_message_pillars_station(
        self, websocket: WebSocket, name_event: str, data: Any
    ):
        await websocket.send_json(BaseMessage(event=name_event, data=data).model_dump())

    async def _send_message_frontend(
        self, websocket: WebSocket, name_event: str, data: Any
    ):
        await websocket.send_json(BaseMessage(event=name_event, data=data).model_dump())
...
class Manager(DefaultManager):
    ...
    async def broadcast_to_pillars_station(
        self, pillar_station_id: uuid.UUID, message, name_event: str
    ):
        websocket = self.pillar_station_connections[pillar_station_id]
        try:
            await self._send_message_pillars_station(websocket, name_event, message)
        except Exception:
            self.pillar_station_connections.pop(pillar_station_id)

    async def broadcast_to_dron_station(
        self, dron_station_id: uuid.UUID, message, name_event: str
    ):
    ...
    async def send_first_message_frontend(self, websocket: WebSocket, pool: Pool):
        async with pool.acquire() as conn:
            await self._send_message_frontend(
                websocket,
                "all_data",
                GreetingMessage(
                    pillars=[
    ...
    async def pillar_station_enter(
        self, websocket: WebSocket, client_id: str, pool: Pool
    ):
        async with pool.acquire() as conn:
            result = await get_pillar_station(conn=conn, id_=uuid.UUID(client_id))
            if result is None:
                await self._send_status_pillar_station(
                    websocket, "Err", "Ошибка в данных"
                )
    ...
    async def _register_drons(self, id_dron_station: uuid.UUID, pool: Pool):
        async with pool.acquire() as coon:
            ...
            if d_s is None:
                await self._send_status_dron_station(
                    self.drone_station_connections[id_dron_station],
                    "Err",
                    "Ошибка в данных",
                )
            ...
            for _ in range(
                ...
                - len(
                    await get_drons_by_station(
                        conn=coon, id_dron_station=id_dron_station
                    )
                )
            ):
                i_drons.append(
    ...
    async def get_drones(self, client_id: uuid.UUID, pool: Pool):
        async with pool.acquire() as coon:
            ...
            for d in await get_drons_by_station(conn=coon, id_dron_station=client_id):
                drons.append(
                    Dron(
                        ...
                        last_coordinates=(
                            None
                            if d.last_latitude is None or d.last_longitude is None
                            else Coordinates(
                                x=int(d.last_latitude),
                                y=int(d.last_longitude),
                            )
            ...
            await self._send_message_dron_station(
                self.drone_station_connections[client_id], "get_drons", drons
            )
    ...
    async def get_pillars(self, client_id: uuid.UUID, pool: Pool):
        async with pool.acquire() as conn:
            ...
            await self._send_message_dron_station(
                self.drone_station_connections[client_id], "get_pillars", pillars
            )
        ...
        async with pool.acquire() as conn:
            ...
            if status == "death" or status == "empty":
                ...
                if result is not None:
                    await self.broadcast_to_dron_station(
                        result,
                        PillarChangeState(id=str(pillar_id), status=status),
                        "change_state_pillar",
                    )
                pillar_station = await get_pillar_station_by_pillar_id(conn, pillar_id)
                if pillar_station is not None:
                    await self.broadcast_to_pillars_station(
                        pillar_station.id,
                        SetModePillar(id=str(pillar_id), is_hit=False, is_service=True),
                        "set_mode_pillar",
                    )
            else:
                pillar_station = await get_pillar_station_by_pillar_id(conn, pillar_id)
                if pillar_station is not None:
                    await self.broadcast_to_pillars_station(
                        pillar_station.id,
                        SetModePillar(
                            id=str(pillar_id), is_hit=False, is_service=False
                        ),
                        "set_mode_pillar",
                    )

        await self.broadcast_to_frontend(
            ChangeStatePillar(id=str(pillar_id), state=status), "change_state_pillar"
        )

    async def change_dron_state(
        self,
        ...
        async with pool.acquire() as conn:
            await update_dron(
                ...
                dron_id,
                status,
                coordinates.x if coordinates is not None else None,
                coordinates.y if coordinates is not None else None,
                dron_station_id,
            )
            await self.broadcast_to_frontend(
                ChangeStateDron(
                    id=str(dron_id), state=status, last_coordinates=coordinates
                ),
                "change_state_dron",
            )
